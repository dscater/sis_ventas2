...
@section('scripts')
    <script>
        ...
        $(document).on('click', 'table.data-table tbody tr td.btns-opciones a.eliminar', function(e) {
            ...
            let registro2 = $(this).parents('tr').children('td').eq(4).text();
            $('#mensajeEliminar').addClass("text-center");
            $('#mensajeEliminar').html(
                `¿Está seguro(a) de eliminar la venta del cliente <b>${registro}</b> con un Total de <b>Bs. ${registro2}</b>?
                <br><h4>¡Esta acción no se podrá deshacer!</h4>`
            );
        ...
        function getVentas(val_url = "") {
            $("#contenedor_registros").html("Cargando...")
            let url = "{{ route('ventas.index') }}" + "?page=1";
            if (val_url != "") {
                url = val_url
            }
            $.ajax({
